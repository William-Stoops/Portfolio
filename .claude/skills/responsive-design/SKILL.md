...
- Every raster image is a `ResponsivePicture` (`src/types/responsive-picture.ts`) whose
  variants are generated by `pnpm images` into `public/images/` (ADR 0012 — not
  `vite-imagetools`, whose dev and build URLs differ and would break prerendered HTML).
- Rendered by `ResponsiveImage` (`src/components/ui/`): `<picture>` with AVIF + WebP
  sources and a JPEG `<img>`, and a **`sizes` attribute that matches the real layout**
  (hero: `(min-width: 64rem) 24rem, (min-width: 40rem) 20rem, 16rem`, mirroring
  `w-64 sm:w-80 lg:w-96`). A wrong `sizes` makes phones download desktop images.
- Explicit `width` / `height` from the picture definition → zero CLS.
- `loading="critical"` for the LCP image (eager, `fetchpriority="high"`); `loading="lazy"`
