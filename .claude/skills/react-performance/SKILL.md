...
- **YouTube**: never an `<iframe>` at load (≈ 500 kB+ and third-party requests).
  `YouTubeFacade` (`src/components/ui/`) renders a button; on activation it mounts the
  `youtube-nocookie.com` player (sandboxed, focused, at the start time) and always offers a
  new-tab link. `e2e/video.spec.ts` asserts no YouTube host is contacted before the click.
