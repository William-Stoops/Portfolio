...
- **YouTube**: never an `<iframe>` at load (≈ 500 kB+ and third-party requests).
  `YouTubeFacade` (`src/components/ui/`) renders a button on the video's first frame,
  served by the site (lazy); on activation the poster grows into a modal `<dialog>`
  covering the viewport (typed View Transition, ADR 0024), and the
  `youtube-nocookie.com` player (sandboxed, at the start time) mounts only while open,
  shown over the poster once loaded — closing unmounts it, which stops the video. It
  always offers a new-tab link. `e2e/video.spec.ts` asserts no YouTube host is contacted
  before the click.
